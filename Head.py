...
#######rejoinee cases were found in Terrier which affects new Joinee count sometimes
#######################################################################
...
def job():
    ...
    cursor = conn.cursor()
    query = "select * from Payroll_Datamart.QMR_Dashboard where StartOfMonth >= '2024-03-01'"
    df_payroll= pd.read_sql(query, con=conn)
    print(df_payroll.columns)
    print('#############')
    ...
    def get_previous_month(date):
        ...
        last_day_of_previous_month = first_day_of_current_month - timedelta(days=30)
    ...
    #cMon='2024-08-01 00:00:00'###'cMon is current month'///previous_month###
    #pMon='2024-07-01 00:00:00'###'pMon is previous month'///month_before_previous#####
    ######in Qpay PAymon=========CMON########################
    #PayMon='August 2024'###'cMon is words' ##########
    #########################
    ...
    #     engine = create_engine(f'mysql+pymysql://user:password@10.225.220.5/Associate_Datamart')
...
# Schedule the job to run every day at 7 PM and check the date
schedule.every().day.at("20:00").do(check_date_and_run)
